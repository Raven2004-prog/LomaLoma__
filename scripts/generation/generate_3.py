...
# --- Enhanced Report Generator with _k suffix ---
def generate_report(doc_id):
    ...
    def record(text, label, style):
        paras.append({'text': text, 'label': label,
    ...
    for i in range(pages):
        ...
        for _ in range(random.randint(4,8)):
            if random.random() < 0.3:
                lvl = random.choices(['H1','H2','H3'], weights=[0.2,0.5,0.3])[0]
                txt = (f"Section {i+1}: {fake.catch_phrase()}" if lvl=='H1' else fake.bs().capitalize())
                sty = {'H1':st_h1,'H2':st_h2,'H3':st_h3}[lvl]
                blocks.append((txt, lvl, sty))
            else:
                blocks.append((fake.paragraph(nb_sentences=random.randint(3,6)), 'BODY', st_body))
        for txt, lvl, sty in blocks:
            record(txt, lvl, sty)
        if i < pages-1: story.append(PageBreak())

    base = f"report_{doc_id}_label"
    suffix = f"_{doc_id}"
    pdf_file = os.path.join(OUT_DIR, base + suffix + ".pdf")
    json_file = os.path.join(OUT_DIR, base + suffix + ".json")
...
# --- ApplicationForm Generator with _k suffix ---
def generate_application_form(doc_id):
    ...
    fields = ["Full Name","Date of Birth","Email Address",
              "Phone Number","Address","Position Applied For","Date"]

    story, paras = [], []
    paras.append({'text':"Application Form", 'label':'H1',
                  'font_size': st_title.fontSize,
                  'relative_font_size': round(st_title.fontSize/BODY_FONT_SIZE,2)})
    story.append(Paragraph("Application Form", st_title))
    story.append(Spacer(1,0.3*inch))

    for fld in fields:
        txt = fld + ":"
        paras.append({'text':txt,'label':'BODY',
                      'font_size':st_body.fontSize,
                      'relative_font_size':round(st_body.fontSize/BODY_FONT_SIZE,2)})
    ...
    sig = "Signature"
    paras.append({'text':sig,'label':'BODY',
                  'font_size':st_body.fontSize,
                  'relative_font_size':round(st_body.fontSize/BODY_FONT_SIZE,2)})
